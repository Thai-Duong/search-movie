...
function MovieCredits() {
  const { movieId } = useParams();
  const { data } = useSWR(
    `https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=6696edaaf6da24b96f29b53d1d64419d`,
    fetcher
  );
  if (!data) return null;
  const { cast } = data;
...
function MovieVideos() {
  const { movieId } = useParams();
  const { data } = useSWR(
    `https://api.themoviedb.org/3/movie/${movieId}/videos?api_key=6696edaaf6da24b96f29b53d1d64419d`,
    fetcher
  );
  if (!data) return null;
  const { results } = data;
...
function MovieSimilar() {
  const { movieId } = useParams();
  const { data } = useSWR(
    `https://api.themoviedb.org/3/movie/${movieId}/similar?api_key=6696edaaf6da24b96f29b53d1d64419d`,
    fetcher
  );
  if (!data) return null;
  const { results } = data;
